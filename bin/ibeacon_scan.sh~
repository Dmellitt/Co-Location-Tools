...
if [[ $1 == "parse" ]]; then
        ...
        do
                ...
                if [ "$capturing" ]; then
					...
					ADDR=`echo $packet | sed 's/^.\{21\}\(.\{17\}\).*$/\1/'`
					...
					ADDR=`echo $ADDR |awk '{for(i=NF;i>1;i--) printf("%s ",$i);printf("%s",$1);}'| sed 's/\ /-/g'`
					...
					TIMESTAMP=`date "+%s"`
                                        if [[ $2 == "-b" ]]; then
                                                ...
                                                echo "TIME: $TIMESTAMP ADDRESS: $ADDR UUID: $UUID MAJOR: $MAJOR MINOR: $MINOR POWER: $POWER RSSI: $RSSI"
                                        fi
                                fi
                                capturing=""
                                packet=""
        ...
        if [ "$(pidof hcitool)" ]; then
                sudo hcidump -i hci0 --raw | $0 parse $1 > /home/pi/data/`date|tr ' ' '-'`.csv
        fi
fi
